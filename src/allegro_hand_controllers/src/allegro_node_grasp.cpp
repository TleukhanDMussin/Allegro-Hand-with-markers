...
double okay_pose[DOF_JOINTS] = 
        {
            -0.21207622141917779, 1.5796173475576383, 0.18424909910568854, -0.004179339421144788, 
            -0.1924500275068717, 1.6007342817901065, 0.14694544021583816, -0.043124332146663565, 
            -0.002837000977515134, -0.06489538866890748, 0.2914052874365268, -0.21898996278356772, 
            1.132937031856528, 1.170386993579141, 0.7950575182387047, 0.02325442107418434
        };

double qucumber_pose[DOF_JOINTS] =
        {
...
void AllegroNodeGrasp::libCmdCallback(const std_msgs::String::ConstPtr &msg) {
  ...
  } else if (lib_cmd.compare("okay") == 0) {
    pBHand->SetJointDesiredPosition(okay_pose);
    pBHand->SetMotionType(eMotionType_JOINT_PD);
  } else if (lib_cmd.compare("qucumber") == 0) {
    pBHand->SetJointDesiredPosition(qucumber_pose);
    pBHand->SetMotionType(eMotionType_JOINT_PD);
  } else {
